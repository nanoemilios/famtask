...
if ($action === 'register') {
    if (!file_exists(CFG)) { jsonOut(['ok'=>false,'msg'=>'Not configured']); exit; }
    handleRegister(getDB(getCfg()));
    exit;
}

// ── Familie verifizieren ──────────────────────────────────────────────────────
if ($action === 'verify') {
    if (!file_exists(CFG)) { jsonOut(['ok'=>false,'msg'=>'Not configured']); exit; }
    handleVerify(getDB(getCfg()));
    exit;
}

// ── Familien-Liste (Admin) ────────────────────────────────────────────────────
if ($action === 'families') {
    if (!file_exists(CFG)) { jsonOut(['ok'=>false,'msg'=>'Not configured']); exit; }
    if (!$isJson) { showAdminPage('families'); exit; }
    handleFamilies(getDB(getCfg()));
    exit;
}
...
function ensureFamilyTable(PDO $pdo): void {
    ...
    // family_code column might not exist yet (old installs)
    try {
        $pdo->exec("ALTER TABLE famtask_data ADD COLUMN IF NOT EXISTS family_code VARCHAR(10) NOT NULL DEFAULT 'default'");
...
function generateCode(PDO $pdo): string {
    $chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // kein O,0,1,I
    do {
        $code = '';
...
// ══════════════════════════════════════════════════════════════════════════════
//  FAMILIE REGISTRIEREN / VERIFIZIEREN
// ══════════════════════════════════════════════════════════════════════════════
...
function handleVerify(PDO $pdo) {
    ...
    $fam = $stmt->fetch();
    if ($fam) {
        jsonOut(['ok'=>true, 'code'=>$fam['code'], 'name'=>$fam['name']]);
    } else {
        jsonOut(['ok'=>false, 'msg'=>'Code nicht gefunden. Bitte prüfen.']);
    }
}
...
function handlePost(PDO $pdo) {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO famtask_data (family_code, data_key, data_value)
                               VALUES (?, ?, ?)
                               ON DUPLICATE KEY UPDATE data_value = VALUES(data_value),
                                                       updated_at = CURRENT_TIMESTAMP");
        $pdo->beginTransaction();
        foreach ($body as $k => $v) {
            $stmt->execute([$fc, (string)$k, is_string($v) ? $v : json_encode($v)]);
        }
        $pdo->commit();
...
function handleTrack(PDO $pdo) {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS famtask_stats (
        id INT AUTO_INCREMENT PRIMARY KEY,
        family_code VARCHAR(10) NOT NULL DEFAULT 'default',
    ...
    $stmt = $pdo->prepare("INSERT INTO famtask_stats (family_code, day, child_id, child_name, event_type, ref_id, ref_label, minutes)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([
        $fc,
        $body['day']        ?? date('Y-m-d'),
...
<div class="card">
  ...
  <div class="section" id="tab-families">
    <h3>Familien verwalten</h3>
    <button class="btn btn-sec" onclick="loadFamilies()" style="margin-bottom:14px">🔄 Aktualisieren</button>
    <div id="fam-loading" style="text-align:center;padding:16px;color:rgba(255,255,255,0.35);display:none">⏳ Lade…</div>
    <div id="fam-list"></div>
    ...
    <input id="fam-name" placeholder="z.B. Familie Müller" style="width:100%;background:#0D0E1A;border:1.5px solid rgba(255,255,255,0.1);border-radius:10px;color:#fff;font-size:14px;padding:11px 14px;outline:none;margin-top:5px"/>
...
<script>
  ...
  async function loadFamilies() {
    document.getElementById('fam-loading').style.display='block';
    document.getElementById('fam-list').innerHTML='';
    try {
      const r = await fetch('api.php?action=families', {headers:{'Accept':'application/json'}});
      const d = await r.json();
      document.getElementById('fam-loading').style.display='none';
      if (!d.ok || !d.families.length) { document.getElementById('fam-list').innerHTML='<p style="color:rgba(255,255,255,0.3);font-size:13px">Noch keine Familien.</p>'; return; }
      document.getElementById('fam-list').innerHTML = d.families.map(f=>`
        <div style="background:#0D0E1A;border-radius:12px;padding:12px 14px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;gap:10px">
          <div>
            <div style="font-weight:800;color:#fff;font-size:13px">${f.name||'Unbenannt'}</div>
            <div style="font-size:11px;color:rgba(255,255,255,0.35);margin-top:2px">${f.data_rows} Datensätze · seit ${new Date(f.created_at).toLocaleDateString('de-CH')}</div>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <code style="background:rgba(255,230,109,0.1);border:1px solid rgba(255,230,109,0.25);border-radius:8px;padding:5px 10px;font-size:15px;font-weight:900;color:#FFE66D;letter-spacing:2px">${f.code}</code>
            <button onclick="navigator.clipboard.writeText('${f.code}')" title="Kopieren" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:rgba(255,255,255,0.5);padding:5px 8px;cursor:pointer;font-size:13px">📋</button>
          </div>
        </div>`).join('');
    } catch(e) {
      document.getElementById('fam-loading').style.display='none';
      document.getElementById('fam-list').innerHTML='<p style="color:#FF6B6B">'+e.message+'</p>';
    }
  }

  async function createFamily() {
    const name = document.getElementById('fam-name').value.trim()||'Meine Familie';
    try {
      const r = await fetch('api.php?action=register', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});
      const d = await r.json();
      if (d.ok) {
        showMsg('fam-msg', true, `✅ Familie "${d.name}" erstellt! Code: ${d.code}`);
        document.getElementById('fam-name').value='';
        loadFamilies();
      } else { showMsg('fam-msg', false, d.msg||'Fehler'); }
    } catch(e) { showMsg('fam-msg', false, e.message); }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    <?php if ($tab === 'families'): ?>loadFamilies();<?php endif; ?>
  });
